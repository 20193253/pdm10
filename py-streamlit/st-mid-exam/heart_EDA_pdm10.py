...
st.sidebar.subheader('Visualization using seaborn and plotly by pdm10')
...
st.header("Histogram")
## multi-column layput
row0_1, row0_space2, row0_2 = st.columns(
    (1, .1, 1))
...
# boxplots
st.header("Boxplot")
st.subheader("With a categorical variable - Outcome [noHD, HD]")
## multi-column layput
...
row2_1, row2_space2, row2_2 = st.columns((1, .1, 1))
...
hmap_params = st.multiselect("Select parameters to include on heatmap", options=list(df.columns),
                             default=[p for p in df.columns if "target" not in p])
